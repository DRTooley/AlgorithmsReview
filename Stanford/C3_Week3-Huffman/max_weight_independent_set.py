...
if __name__ == '__main__':
    with open('mwis.txt') as mwis:
        vertex_count = mwis.readline()
        path_graph = [int(weight) for weight in mwis]

    memory = []

    for i, w in enumerate(path_graph):
        if i == 0:
            memory.append(w)

        elif i == 1:
            memory.append(max(w, memory[0]))

        else:
            included = w + memory[-2]
            excluded = memory[-1]
            memory.append(max(included, excluded))
            if (i >= 116 and i <= 124) or \
                    (i >= 516 and i <= 524) or \
                    (i >= 996) or \
                    (i >= 16 and i <= 24):
                print("{} ".format(i), end='')
                if memory[-1] == included:
                    print("Included")
                else:
                    print("Excluded")
